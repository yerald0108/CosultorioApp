...
import { GestanteData, ClasificacionRiesgo, calcularFPP, calcularEdadGestacional } from '../types/PAMITypes';
import { PAMIService } from '../services/PAMIService';
import ClasificacionSelector from '../components/ClasificacionSelector';
import { styles } from '../styles/screens/CrearGestanteScreen.styles';

type CrearGestanteScreenNavigationProp = StackNavigationProp<RootStackParamList>;

interface Props {
  navigation: CrearGestanteScreenNavigationProp;
}

const CrearGestanteScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const calcularFUMDesdeEG = (edadGestacional: number): Date | null => {
    try {
      const hoy = new Date();
      const diasGestacion = Math.floor(edadGestacional * 7); // Convertir semanas a días
  ...
  // Función para formatear fecha de nacimiento (igual que FUM)
  ...
  // Función para generar carnet automáticamente desde fecha nacimiento
  ...
  const handleFechaNacimientoChange = (texto: string) => {
    ...
    if (fechaFormateada.length === 10) {
      ...
      if (carnetParcial) {
        // Mantener los últimos 5 dígitos si ya existen
  ...
  // Función para parsear fecha (reutilizar la de FUM)
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      if (formData.fum.trim()) {
        fumDate = parsearFechaFUM(formData.fum)!;
        ...
        fumDate = calcularFUMDesdeEG(edadGestacional)!;
      }
      
      const fppDate = calcularFPP(fumDate);

      const gestanteData: Omit<GestanteData, 'id' | 'fechaCreacion' | 'fechaActualizacion'> = {
      ...
      const resultado = await PAMIService.crearGestante(gestanteData);

      if (resultado.success) {
        Alert.alert(
          'Éxito',
          'Gestante creada correctamente',
          [
            {
              text: 'Aceptar',
        ...
        Alert.alert('Error', resultado.error || 'No se pudo crear la gestante');
      }
    } catch (error) {
      console.error('Error creando gestante:', error);
      Alert.alert('Error', 'Error técnico al crear la gestante');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      <ScrollView style={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerSubtitle}>Completa todos los campos obligatorios</Text>
        </View>
            ...
            <View style={styles.section}>
              ...
              <View style={styles.calculatedInfo}>
                <View style={styles.calculatedItem}>
                  <Text style={styles.calculatedLabel}>📅 FPP Calculada:</Text>
...
export default CrearGestanteScreen;
